                    ...
                    <ul class="nav nav-tabs nav-tabs-bordered" id="borderedTab" role="tablist">
                        ...
                        <li class="nav-item" role="presentation">
                          <button class="nav-link" id="profile-tab" data-bs-toggle="tab" data-bs-target="#bordered-profile"
                            type="button" role="tab" aria-controls="profile" aria-selected="false">Bayar</button>
                        </li>
                    </ul>
                    <!-- ISI -->
                    <div class="tab-content pt-2" id="borderedTabContent">
                        <div class="tab-pane fade show active" id="bordered-home" role="tabpanel"
                            ...
                            <div class="row">
                              ...
                              <div class="col-3">
                                <div class="card">
                                  <img src="/uploads/gambar/ram/{{$memories->memoryImage }}" class="card-img-top" alt="">
                                  <div class="card-body">
                                    <h6 class="card-text ">{{$memories->memoryName }}</h6>
                                    <p class="card-text">Rp. {{ $memories->memoryPrice }}</p>
                                    <form method="POST" action="{{ route('cart.add') }}">
                                      @csrf
                                      <input type="hidden" name="productId" value="{{$memories->memoryId }}">
                              ...
                              <div class="col-3">
                                <div class="card">
                                  <img src="/uploads/gambar/storage/{{$storages->storageImage }}" class="card-img-top" alt="">
                                  <div class="card-body">
                                    <h6 class="card-text ">{{$storages->storageName }}</h6>
                                    <p class="card-text">Rp. {{ $storages->storagePrice }}</p>
                                    <form method="POST" action="{{ route('cart.add') }}">
                                      @csrf
                                      <input type="hidden" name="productId" value="{{$storages->storageId }}">
                                      <button type="submit">Tambahkan ke Keranjang</button>
                                    </form>
                                  </div>
                                </div>
                            ...
                            <div class="row col">
                                @foreach ($pelanggan as $data)
                                  ...
                                  <div class="col-sm-10 mb-2">
                                    <input class="form-control" type="text" name="" value="{{$data->customerName}}" disabled>
                                    <input class="form-control" type="hidden" name="pelanggan" value="{{$data->customerName}}">
                                    <input class="form-control" type="hidden" name="idpelanggan" value="{{$data->customerId}}">
                                  </div>
                                  ...
                                  <div class="col-sm-10 mb-2">
                                    <input class="form-control" type="number" name="" value="{{$data->customerPhoneNumber}}" disabled>
                                    <input class="form-control" type="hidden" name="telp" value="{{$data->customerPhoneNumber}}">
                                  </div>
                                @endforeach
